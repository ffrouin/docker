...
cp $pmapath2 /root/config.sample.inc.php.old
...
echo "Creating PMA DB access"
#SOME WORK with DATABASE (table / user)
PMADB=phpmyadmin
PMAUSER=pma

echo "	drop existing PMA data"
#DROP USER and TABLE
mysql -uroot -padmin<<MYSQL_PMA1
DROP USER '$PMAUSER'@'localhost';
DROP DATABASE $PMADB;
FLUSH PRIVILEGES;
MYSQL_PMA1

echo "	create PMA user"
#CREATE PMA USER
mysql -uroot -padmin<<MYSQL_PMA2
CREATE USER '$PMAUSER'@'localhost' IDENTIFIED BY '$PASS';
CREATE DATABASE $PMADB;
MYSQL_PMA2

echo "	grant rihghts to PMA user"
#GRANT PMA USE SOME RIGHTS
mysql -uroot -padmin<<MYSQL_PMA3
USE $PMADB;
...
echo "	create phpmyadmin tables"
mysql -uroot -padmin < /root/create_tables.sql

echo "	store pma password (/root/pma.txt)"
echo "pma@localhost user password = $PASS" > /root/pma.txt
echo "	done"
